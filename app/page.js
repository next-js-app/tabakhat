...
export default function Home() {
  ...
  return (
    <main className="min-h-screen">
      {/* Hero Section */}
      <section className="relative h-auto md:h-[60vh] flex flex-col md:flex-row items-center justify-center text-white bg-black/80">
          ...
          >
            {customerReviews.map((review) => (
              <SwiperSlide key={review.id}>
                <Card className="p-6 h-full flex flex-col justify-between items-center text-center bg-white border border-gray-200 rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300">
                  <div className="mb-4">
                    <Image
                      src={review.imageUrl}
                      alt={review.name}
                      width={80}
                      height={80}
                      className="rounded-full object-cover border-2 border-[#FFE353]"
                    />
                  </div>
                  <CardDescription className="text-lg mb-4 flex-grow">“{review.review}”</CardDescription>
      ...
      {/* CTA Section */}
      <section className="py-16 px-4 bg-[#FFE353]">
        <div className="max-w-7xl mx-auto text-center">
          ...
          >
            <Link
              href="/recipes"
              className="bg-black text-white px-8 py-3 rounded-full text-lg font-semibold hover:bg-black/90 transition-colors"
            >
              Get Started
            </Link>
          </motion.div>
